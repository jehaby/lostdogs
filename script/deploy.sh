...
Optional:
  ...
  --remote-path DIR Remote directory on the host (default: /tmp)
...
REMOTE_ENV_FILE="${REMOTE_PATH%/}/lostdogs.env"
REMOTE_COMPOSE_FILE="${REMOTE_PATH%/}/lostdogs.compose.yml"

echo "1) Ensuring local image exists: ${IMAGE}"
docker image inspect "${IMAGE}" >/dev/null 2>&1 || { echo "Image ${IMAGE} not found locally" >&2; exit 2; }
...
echo "  b) Compose up"
docker compose --env-file "${REMOTE_ENV_FILE}" -f "${REMOTE_COMPOSE_FILE}" up -d

echo "  c) Cleanup image tar"
rm -f "${IMAGE_FILENAME}"
REMOTE_SCRIPT
